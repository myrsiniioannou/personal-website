...
const getPortfolioProjectTitlesSortedByDate = () => {
    ...
    folders.forEach(folder => {
        ...
        const project = {folder, name: "", projectNumber: 0, firstImageForCover: ""};
        files.forEach(file => {
    ...
    let sortedData = allProjects.sort((prev, curr) => curr.projectNumber - prev.projectNumber);
    return sortedData
};
...
const HomePage = () => {
    ...
    const portfolioProject = portfolioProjectsMetaData.map((project) => (
        <Link href={"projects/" + project.name}>
            <li className="font-bold xl:text-7xl lg:text-6xl md:text-5xl list-none hover:text-neon">{project.name.replaceAll('-', " ")}</li>
        </Link>
    ));
    return <div className="flex flex-col h-full justify-center text-3xl lg:text-5xl py-14 lg:py-0">
                {portfolioProject}
            </div>;
};
